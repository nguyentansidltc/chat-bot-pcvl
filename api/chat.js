...
export default async function handler(req) {
  try {
    ...
    const systemPrompt = `
      ...
      3) Phải tính tổng tiền trước VAT (tính đúng số tiền, không bịa số).
      4) VAT = 8% × tiền trước VAT (tính đúng số tiền, không bịa số).
      5) Tổng cộng = tiền trước VAT + VAT (cộng đúng số tiền, không bịa số).
      6) Khi trả lời, hiển thị:
        - Tiền trước VAT (hiển thị đúng số tiền).
        - VAT 8% (hiển thị đúng số tiền).
        - Tổng cộng sau VAT (hiển thị đúng số tiền).
